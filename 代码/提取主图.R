...
modern_paired_plot_with_axis_sig <- function(p_name, y_title, unit = NULL, lines = NULL, tag_label) {
  ...
  p <- ggplot(df, aes(x = Method, y = Value)) +
    # 样本连线
    geom_line(aes(group = Sample), color = "grey90", linewidth = 0.8) +  # 增加连线粗细
    # 箱线图
    geom_boxplot(aes(fill = Method), width = 0.4, color = "black", alpha = 0.7, 
                 outlier.shape = NA, linewidth = 1.2) +  # 增加箱线图边框粗细
    # 散点
    geom_point(aes(color = Method), size = 3.5) +  # 增加散点大小
    # 颜色
    ...
    theme(legend.position = "none", 
          axis.title = element_text(face = "bold", size = 16, family = "sans"),  # 增大坐标轴标题
          axis.title.x = element_blank(),
          axis.text.x = element_text(size = 16, face = "bold", family = "sans"),  # 增大X轴文字
          axis.text.y = element_text(size = 16, face = "bold", family = "sans"),  # 增大Y轴文字
          axis.line = element_line(color = "black", linewidth = 1.2),  # 增大坐标轴线粗细
          axis.ticks = element_line(color = "black", linewidth = 1.2),  # 增大刻度线粗细
          axis.ticks.length = unit(0.3, "cm"),  # 增大刻度线长度
          plot.tag = element_text(size = 24, face = "bold", margin = margin(0, 0, 0, 8)),  # 增大子图标签
          plot.margin = margin(10, 10, 10, 10))  # 增大边距
  
  # 添加显著性标记（增大横线和星号大小）
  p <- p +
    # 显著性横线
    annotate("segment", x = 1, xend = 2, y = y_sig, yend = y_sig, 
             linewidth = 1.2) +  # 增大横线粗细
    # 显著性标记文本
    annotate("text", x = 1.5, y = y_sig + 0.05 * (y_max - y_min), 
             label = sig_label, size = 7, fontface = "bold")  # 增大星号大小
    ...
    if(p_name == "A260_230") {
      ...
      p <- p +
        annotate("segment", x = 0.5, xend = 2.5, y = 2.0, yend = 2.0, 
                 linetype = "dotted", color = "grey40", linewidth = 1.0)  # 增大参考线粗细
    } else if(p_name == "A260_280") {
      # A260/280: 1.7和2.0两条线
      p <- p +
        annotate("segment", x = 0.5, xend = 2.5, y = 1.7, yend = 1.7, 
                 linetype = "dotted", color = "grey40", linewidth = 1.0) +
        annotate("segment", x = 0.5, xend = 2.5, y = 2.0, yend = 2.0, 
                 linetype = "dotted", color = "grey40", linewidth = 1.0)
    } else {
      # 其他参数
...
ggsave("Figure_1_DNA_Quality_Large.pdf", panel_1, width = 16, height = 9, device = cairo_pdf, dpi = 600)
...
sig_height <- y_max + 8  # 增大间距

p_f <- ggplot(df_long, aes(x = X, y = Value)) +
  # 样本连线
  geom_line(aes(group = interaction(Sample, Stain)), color = "grey90", linewidth = 0.8) +
  # 箱线图
  geom_boxplot(aes(fill = Method), width = 0.5, color = "black", alpha = 0.7, 
               outlier.shape = NA, linewidth = 1.2) +  # 增大箱线图边框
  # 散点
  geom_point(aes(color = Method), size = 3.5) +  # 增大散点
  # 其他图形元素
  scale_x_discrete(labels = c("CP", "CZ", "CP", "CZ")) +
  scale_fill_manual(values = c("CP" = "#08519C", "CZ" = "#EF6548")) +
  scale_color_manual(values = c("CP" = "#08519C", "CZ" = "#EF6548")) +
  geom_vline(xintercept = 2.5, linetype = "dashed", color = "grey50", linewidth = 1.0) +  # 增大参考线
  annotate("text", x = 1.5, y = y_max + 18, label = "Gram-positive",  # 增大分组标签间距
           size = 8, fontface = "bold") +  # 增大分组标签
  annotate("text", x = 3.5, y = y_max + 18, label = "Gram-negative", 
  ...
  theme(
    ...
    axis.text.x = element_text(size = 16, face = "bold", family = "sans"),  # 增大X轴文字
    axis.text.y = element_text(size = 16, face = "bold", family = "sans"),  # 增大Y轴文字
    axis.line = element_line(color = "black", linewidth = 1.2),  # 增大坐标轴线
    axis.ticks = element_line(color = "black", linewidth = 1.2),  # 增大刻度线
    axis.ticks.length = unit(0.3, "cm"),  # 增大刻度长度
    plot.tag = element_text(size = 24, face = "bold", margin = margin(0, 0, 0, 8)),  # 增大子图标签
    plot.margin = margin(10, 10, 10, 10)
  ) +
  ylim(0, y_max + 20)  # 增大上限以适应更大标签

# 添加显著性标记 - 增大所有元素
p_f <- p_f +
  annotate("segment", x = 1, xend = 2, y = sig_height, yend = sig_height, 
           linewidth = 1.2) +  # 增大横线
  annotate("text", x = 1.5, y = sig_height + 3, label = gp_label, size = 8) +  # 增大星号
  annotate("segment", x = 3, xend = 4, y = sig_height, yend = sig_height, 
           linewidth = 1.2) +
  annotate("text", x = 3.5, y = sig_height + 3, label = gn_label, size = 8)

ggsave("Figure_2_Gram_Large.pdf", p_f, width = 10, height = 7, device = cairo_pdf, dpi = 600)
...
tryCatch({
  ...
  create_div_plot_with_sig <- function(met, y_lab, tag_lab) {
    ...
    p <- ggplot(df, aes(x = Method, y = Value, fill = Method)) +
      geom_line(aes(group = sample_name), color = "grey85", linewidth = 0.8) +  # 增大连线
      geom_boxplot(width = 0.4, color = "black", alpha = 0.7, 
                   outlier.shape = NA, linewidth = 1.2) +  # 增大箱线图边框
      geom_point(shape = 21, size = 4.0, color = "black", stroke = 0.8, alpha = 0.7) +  # 增大点
      scale_fill_manual(values = c("CP" = "#08519C", "CZ" = "#EF6548")) +
    ...
    p <- p +
      annotate("segment", x = 1, xend = 2, y = y_sig, yend = y_sig, 
               linewidth = 1.2) +  # 增大横线
      annotate("text", x = 1.5, y = y_sig + 0.05 * (y_max - y_min), 
               label = sig_label, size = 8, fontface = "bold")  # 增大星号
  ...
  ggsave("Figure_3_Diversity_Large.pdf", panel_3, width = 16, height = 7, device = cairo_pdf, dpi = 600)
  ...
  p_g <- ggplot() + 
    theme_void() +
    labs(tag = "G") +
    theme(plot.tag = element_text(size = 24, face = "bold", margin = margin(0, 0, 0, 8)))
  p_h <- ggplot() + 
    theme_void() +
    labs(tag = "H") +
    theme(plot.tag = element_text(size = 24, face = "bold", margin = margin(0, 0, 0, 8)))
...
tryCatch({
  # 5.1 数据处理
  data <- read.csv("../表格/提取主图_短读数据bray_distance.csv")
  ...
  p_i <- ggplot(df_plot, aes(x = Group, y = Distance)) +
    geom_boxplot(aes(color = Group), fill = "white", width = 0.5, 
                 linewidth = 1.5, outlier.shape = NA) +  # 增大箱线图边框
    geom_jitter(aes(fill = Group, color = Group), width = 0.15, size = 4.0,  # 增大散点
                shape = 21, stroke = 0.8, alpha = 0.7) +
    # 其他图形元素
    ...
    theme(
      ...
      axis.text.x = element_text(size = 16, face = "bold", family = "sans"),  # 增大X轴文字
      axis.text.y = element_text(size = 16, face = "bold", family = "sans"),  # 增大Y轴文字
      axis.title.y = element_text(size = 16, face = "bold", family = "sans"),  # 增大Y轴标题
      axis.title.x = element_blank(),
      legend.position = "none",
      plot.margin = margin(10, 10, 10, 10)
    ) +
    ylim(0, y_max + 0.25)  # 增大上限以适应更大标签
  
  # 添加显著性标记 - 增大所有元素
  p_i <- p_i +
    annotate("segment", x = 1, xend = 2, y = y_max + 0.05, yend = y_max + 0.05,
             linewidth = 1.2) +  # 增大横线
    annotate("text", x = 1.5, y = y_max + 0.08, label = lab1, size = 8, fontface = "bold") +  # 增大星号
    annotate("segment", x = 2, xend = 3, y = y_max + 0.12, yend = y_max + 0.12,
             linewidth = 1.2) +
    annotate("text", x = 2.5, y = y_max + 0.15, label = lab2, size = 8, fontface = "bold") +
    annotate("segment", x = 1, xend = 3, y = y_max + 0.20, yend = y_max + 0.20,
             linewidth = 1.2) +
    annotate("text", x = 2, y = y_max + 0.23, label = lab3, size = 8, fontface = "bold")
  
  ggsave("Figure_4_BrayCurtis_Large.pdf", p_i, width = 10, height = 8, device = cairo_pdf, dpi = 600)
...
create_placeholder <- function(label) {
  ggplot() + 
    annotate("text", x = 0.5, y = 0.5, label = paste("Panel", label), size = 16) +  # 增大占位符文字
    theme_void() +
    labs(tag = label) +
    theme(plot.tag = element_text(size = 24, face = "bold", margin = margin(0, 0, 0, 8)))  # 增大子图标签
}
...
# 保存最终图形
ggsave("Figure_Complete_A_to_I_Large.pdf", final_figure, 
       width = 20, height = 20, device = cairo_pdf, dpi = 600)  # 增大完整图尺寸
ggsave("Figure_Complete_A_to_I_Large.png", final_figure, 
       width = 20, height = 20, dpi = 600, bg = "white")

cat("所有图形生成完成！\n")
cat("已保存文件:\n")
cat("1. Figure_1_DNA_Quality_Large.pdf (A-E)\n")
cat("2. Figure_2_Gram_Large.pdf (F)\n")
cat("3. Figure_3_Diversity_Large.pdf (G-H)\n")
cat("4. Figure_4_BrayCurtis_Large.pdf (I)\n")
cat("5. Figure_Complete_A_to_I_Large.pdf (完整图A-I)\n")
cat("6. Figure_Complete_A_to_I_Large.png (完整图A-I)\n")